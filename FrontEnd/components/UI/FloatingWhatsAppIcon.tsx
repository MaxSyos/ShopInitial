...
const FloatingWhatsAppIcon: React.FC = () => {
  ...
  useEffect(() => {
    const savedPosition = localStorage.getItem('whatsappIconPosition');
    if (savedPosition) {
    ...
    const fetchPhoneNumber = async () => {
      try {
        const res = await api.get('/content/whatsapp');
        if (res.data.item?.value) {
          setPhoneNumber(res.data.item.value);
        }
      } catch (error) {
  ...
  const handleMouseUp = () => {
    if (isDragging) {
      ...
      localStorage.setItem('whatsappIconPosition', JSON.stringify(position));
    }
  };
  ...
  return (
    ...
    >
      <div className="bg-green-500 hover:bg-green-600 rounded-full p-3 shadow-lg transition-colors">
        <svg
