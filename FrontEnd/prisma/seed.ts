...
async function main() {
  ...
  // Limpa coleções (apenas em ambiente de desenvolvimento)
  ...
  const categories = [parentCategory, subCategory, phonesCategory];

  // Função utilitária para gerar número aleatório
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    await prisma.image.createMany({
      data: [
        { url: `https://picsum.photos/seed/product${i}/640/480`, alt: `Imagem do produto ${i}`, productId: product.id },
      ],
    });
